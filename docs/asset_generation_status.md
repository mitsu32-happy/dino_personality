...
- 質問画面用の生成アセットを追加。
  - `assets/images/ui/question-panel.webp`
  - `assets/images/ui/option-card.webp`
- 選択肢カードは、HTML側の `A/C/Q/S/V` 円形フレームと干渉しないよう、円形装飾なしのアセットへ再生成。
- 質問画面下部のマスコットをトップページの博士マスコットへ差し替え、右側配置に変更。
- 上部メニューを診断/結果/図鑑画面でも背景画像内に収まる位置へ調整。
- サイトアイコンを左上のホームボタンと同じ生成アセットへ差し替え。
- 24問すべての質問文について、読みやすい位置で改行されるようID別の改行指定を追加。
- 診断画面の戻るボタンが1クリックで複数問戻らないよう、クリック処理の重複を修正。

## 2026-05-22 結果画面/図鑑高品質化

### 完了

- 図鑑カード専用のリアル寄り恐竜アセットを16タイプ分生成。
  - `assets/images/dex_cards/{type_id}.webp`
- 結果画面/保存画像用のタイプ別マスコットを16タイプ分再生成し、クロマキー除去で透過WebP化。
  - 既存の丸みのある画風を維持。
  - リアル寄り恐竜と構図が被らないよう、立ち/座り/小物持ちなどのポーズに変更。
- 図鑑経由の結果表示では「結果画像を保存」「もう一度診断」「図鑑を見る」を非表示に変更。
- 結果カード下部の「あなたの強み」欄をカード内下部固定に変更し、全16タイプで見切れないことを確認。
- `assets/images/manifest.json` に `dexCards` を追加し、図鑑カード表示を専用アセットへ切り替え。

### 検証

- `node --check .\src\app.js`
- 390px幅で図鑑カード16枚の反映を確認。
- 図鑑経由の結果画面でアクションボタン非表示を確認。
- 全16タイプの結果画面で「あなたの強み」欄がカード枠内に収まることを確認。
...
## 2026-05-22 トップページ調整2

### 完了

- タイトルロゴを背景プレートなしの透過ロゴとして再生成。
- 博士マスコットをタイプ別マスコットに近い、丸い3Dイラスト調へ再生成。

### 実装反映

- トップページのロゴ配置をUIモックに近い上部寄りへ調整。
- マスコットと説明文・CTAが重ならないよう、博士マスコットを下部右側へ再配置。

## 2026-05-22 トップページ調整3

### 完了

- タイトルロゴを左右中央寄せに調整。
- トップ画面では上部メニューを画面内HUDとして配置。
- 診断開始ボタンをタイトルロゴと博士マスコットの間に配置。
- サブタイトルを博士マスコット左側の吹き出しへ移動。
- 背景プレートありの旧 `title-plate` は未使用になったため削除。

## 2026-05-22 トップページ調整4

### 完了

- 下部の密度改善案として `docs/mockups/home-lower-quality-mock.png` をイラスト生成。
- モック方針に合わせ、トップ専用HUDを背景カード内に配置。
- 診断開始ボタン下の余白を埋める発掘台座アセット `home-pedestal` を生成し、下部左側に配置。
- 博士マスコットと吹き出しを、下部台座とバランスする位置へ再配置。

## 2026-05-22 トップページ調整5

### 完了

- UIモックに寄せてタイトルロゴを再生成。
- UIモックに寄せてトップ専用の診断開始ボタン背景 `home-start-button` を生成。
- 画面全体のバランスを見ながら、ロゴ、開始ボタン、下部台座、博士マスコット、吹き出しを再配置。
- 吹き出し内コメントを「あなたの中に眠る、 / 古代の個性を発掘しよう！」に変更。
- 右上HUDを右寄せのまま背景カード内へ収め、開始ボタンの外周枠と説明テキスト位置を調整。
